    ...
    public class CategoryHandler(IHttpClientFactory httpClientFactory) : ICategoryHandler
    {
        ...
        public async Task<Response<Category?>> CreateAsync(CreateCategoryRequest request)
        {
            ...
            return await result.Content.ReadFromJsonAsync<Response<Category?>>()
                ?? new Response<Category?>(data:null, code:400, message:"Falha ao criar categoria");
        }
        ...
        public async Task<PagedResponse<List<Category>?>> GetAllAsync(GetAllCategoriesRequest request)
            => await _client.GetFromJsonAsync<PagedResponse<List<Category>?>>("v1/categories")
               ?? new PagedResponse<List<Category>?>(data: null, code: 400, message: "Falha ao criar categoria");
    }
}
